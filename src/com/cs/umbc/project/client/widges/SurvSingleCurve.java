...
public class SurvSingleCurve extends SimplePlot implements ClickHandler{

	@UiField(provided = true)
	private static SurvSingleCurve plot = null;
 ...
 static int ss =0;
	...
	public static void setRadioButton(List<RadioButton> rb) {
		SurvSingleCurve.rb = rb;
	}
	public static void setCurves(List<Curve> curves) {
		SurvSingleCurve.curves = curves;
	}

	private static double maxTime;
	private static int step;

	public SurvSingleCurve(PlotModel plotModel, PlotOptions plotOptions) {
		super(plotModel, plotOptions);
	}

	public static SurvSingleCurve createEmptyPlot() {
		initial();
		plot.setData();
		return plot;
	}

	public static SurvSingleCurve createPlot(Curve[] list, double max, int i) {
	...
	private static void drawCurves(int m) {
...
//			System.out.println("label: "+curves.get(i).getLabel());
         ...
         // GET
			plot.getModel().addSeries(curves.get(i).getLabel()+"/"+curve.getTime().length);
					...
					SeriesHandler series = plot.getModel().getHandlers().get(m);
					
			Curve curve = curves.get(m);
	...
	public static SurvSingleCurve createPlot() {
		initial();
		plot.generateRandomData();
		return plot;
	}
	...
	public static void initial() {
		...
		plot = new SurvSingleCurve(model, plotOptions);
		Window.alert("ss");
